...
LAppLive2DManager.prototype.changeModel = function(gl)
{
    ...
    if (this.reloadFlg)
    {
        ...
        var no = parseInt(this.count % LAppDefine.MODEL_NUM);
        ...
        switch (no)
        {
            case 0: // 女异人
                ...
                this.createModel();
                this.models[0].load(gl, LAppDefine.MODEL_NVYIREN);
                break;
            case 1: // 男刀客
                this.releaseModel(0, gl);
                this.createModel();
                this.models[0].load(gl, LAppDefine.MODEL_NANDAO);
                break;
            case 2: // ハル
...
{    
    ...
    for (var i = 0; i < this.models.length; i++)
    {
        ...
        if (this.models[i].hitTest(LAppDefine.HIT_AREA_HEAD, x, y))
        {
            ...
            this.models[i].setRandomExpression(LAppDefine.MOTION_GROUP_FLICK_HEAD);
        }
        ...
        else if (this.models[i].hitTest(LAppDefine.HIT_AREA_SKIRT, x, y))
        {
            // 体をタップしたらモーション
            if (LAppDefine.DEBUG_LOG)
                console.log("Tap Skirt." + " models[" + i + "]");

            this.models[i].startRandomMotion(LAppDefine.MOTION_GROUP_TAP_SKIRT,
                                             LAppDefine.PRIORITY_NORMAL);
        }
        else if (this.models[i].hitTest(LAppDefine.HIT_AREA_UMBRELLA, x, y))
        {
            // 体をタップしたらモーション
            if (LAppDefine.DEBUG_LOG)
                console.log("Tap Skirt." + " models[" + i + "]");

            this.models[i].startRandomMotion(LAppDefine.MOTION_GROUP_TAP_UMBRELLA,
                                             LAppDefine.PRIORITY_NORMAL);
        }
    }

 ...
 * 表情をランダムに切り替える
 */
LAppModel.prototype.setRandomExpression = function(motionName)
{
    var tmp = [];
    ...
    var soundName = this.modelSetting.getMotionSound(motionName, 0);
    if (soundName) {
...
LAppModel.prototype.playSound = function(soundPath)
{
    ...
    if (this.options.audioPlayer) {
        ...
        if (AudioContext && this.options.lipSyncWithSound) {
            ...
            audio.intervalId = setInterval(function () {
                if (Date.now() - lastTime  < 33) {
                    ...
                    cache.length && (thisRef.lipSyncValue = (cache.reduce(function (previous, current) { return current += previous }) / cache.length / 100));
                    lastTime = Date.now();
                    cache = [];
                }
            }, 0);
...
LAppModel.prototype.stopSound = function() {
...
LAppModel.prototype.setExpression = function(name)
{
    ...
    this.expressionManager && this.expressionManager.startMotion(motion, false);
}
